...
UserSchema.methods.generateAuthToken = function(){
    ...
    var access = 'auth';
    var token = jwt.sign({_id: user._id.toHexString(), access}, process.env.JWT_SECRET).toString(); //removed secret code and replace with process.env.JWT_SECRET from config.json 
...
UserSchema.statics.findByToken = function(token) {
    ...
    //try-catch block
    try{
        decoded = jwt.verify(token, process.env.JWT_SECRET)//removed secret code and replace with process.env.JWT_SECRET from config.json 
    }catch(e){
